...
if [ "$target" == "esp32" -o "$target" == "esp32s3" -o "$target" == "unphone" -o "$target" == "esp32-small" ]; then
	...
	if [ "$target" == "esp32" ]; then
		...
		# --enable-uart-repl={y/n}: This allows you to turn on and off the UART based REPL.
		# NOTE: Disabled for boards that have native USB (USB-JTAG/CDC accessible).
		# Boards that only have a USB-UART bridge (e.g. Crowpanel) need UART REPL enabled,
		# otherwise all MicroPython output is redirected to USB-CDC (ttyACM0) and becomes invisible.
